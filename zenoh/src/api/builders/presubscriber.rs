...
#[derive(Debug)]
pub struct PreSubscriberBuilder<'a, 'b, Handler, const BACKGROUND: bool = false> {
...
impl<'a, 'b> PreSubscriberBuilder<'a, 'b, DefaultHandler> {
    ...
    /// ```
    #[inline]
    pub fn callback<F>(self, callback: F) -> PreSubscriberBuilder<'a, 'b, Callback<Sample>>
    where
    ...
    /// If your callback is also accepted by the [`callback`](PreSubscriberBuilder::callback) method, we suggest you use it instead of `callback_mut`.
    ///
    /// # Examples
    /// ```
    /// # #[tokio::main]
    ...
    /// ```
    #[inline]
    pub fn callback_mut<F>(self, callback: F) -> PreSubscriberBuilder<'a, 'b, Callback<Sample>>
    where
    ...
    /// ```
    #[inline]
    pub fn with<Handler>(self, handler: Handler) -> PreSubscriberBuilder<'a, 'b, Handler>
    where
        Handler: IntoHandler<Sample>,
    {
        let PreSubscriberBuilder {
            session,
            key_expr,
            origin,
            handler: _,
...
impl<'a, 'b> PreSubscriberBuilder<'a, 'b, Callback<Sample>> {
    ...
    /// # }
    /// ```
    pub fn background(self) -> PreSubscriberBuilder<'a, 'b, Callback<Sample>, true> {
        PreSubscriberBuilder {
...
impl<Handler, const BACKGROUND: bool> PreSubscriberBuilder<'_, '_, Handler, BACKGROUND> {
...
impl<Handler> Resolvable for PreSubscriberBuilder<'_, '_, Handler>
where
...
impl<Handler> Wait for PreSubscriberBuilder<'_, '_, Handler>
where
    Handler: IntoHandler<Sample> + Send,
    Handler::Handler: Send,
{
    fn wait(self) -> <Self as Resolvable>::To {
        ...
        let estimated_time:Duration = Duration::from_secs(1);
        session
            .0
            .declare_presubscriber_inner(&key_expr, self.origin, callback, estimated_time)
...
impl<Handler> IntoFuture for PreSubscriberBuilder<'_, '_, Handler>
where
    Handler: IntoHandler<Sample> + Send,
    Handler::Handler: Send,
{
...
impl IntoFuture for PreSubscriberBuilder<'_, '_, Callback<Sample>, true> {
    type Output = <Self as Resolvable>::To;
    type IntoFuture = Ready<<Self as Resolvable>::To>;
