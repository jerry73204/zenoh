...
| D | **Starvation + preemption flap** under churn — `≺` lets high-prio churn starve low; preemption set changes every DB change. | 🟠 | mitigation-known | immutable `(prio,ZID)` order; min holding time; aging. See also [oscillation S3](oscillation.md) |
