...
| [zenoh-routing-readiness.md](zenoh-routing-readiness.md) | **Review** of Zenoh's actual routing against 4 goals (deterministic / bounded-latency / BW-guaranteed / mixed-flow): what the code does today, per-goal blockers + minimal change, the shared substrate, and a scoped verdict + first-cut plan. Start here for the gap analysis. |
