...
impl IsotpEndpoint {
    pub(crate) fn parse(endpoint: &EndPoint) -> ZResult<IsotpEndpoint> {
        if endpoint.protocol().as_str() != ISOTP_LOCATOR_PREFIX {
            bail!(
                "not an ISO-TP locator: {endpoint} (expected protocol `{ISOTP_LOCATOR_PREFIX}`)"
            );
        }
        ...
        let prio_classes = get_u32(endpoint, config::PRIO_CLASSES)?
            .unwrap_or(DEFAULT_PRIO_CLASSES as u32);
...
mod tests {
    ...
    fn class_zero_is_the_most_urgent_and_holds_the_lowest_identifier() {
        let c =
            IsotpEndpoint::parse(&ep("isotp/vcan0#tx_id=0x100;rx_id=0x200;prio_classes=8")).unwrap();
