...
/// a peer's traffic. See RFC-0080 §4.2.
pub mod config {
...
fn parse_u32(key: &str, s: &str) -> ZResult<u32> {
    ...
    parsed.map_err(|e| zenoh_result::zerror!("invalid `{key}` value {s:?} on a CAN locator: {e}").into())
}
...
mod tests {
    ...
    fn an_id_outside_its_own_band_is_refused() {
        let e =
            CanEndpoint::parse(&ep("can/vcan0#id=0x201;match=0x100;mask=0x700")).unwrap_err();
